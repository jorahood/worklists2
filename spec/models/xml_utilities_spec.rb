...
describe XmlUtilities do
  ...
  context "cloning all v1 lists" do
    before do
      ...
      @list = Factory.build(:list,:wl1_clone => '1')
      @list2 = Factory.build(:list,:wl1_clone => '2')
      @list3 = Factory.build(:list,:wl1_clone => '3')
    ...
    it "should get all v1 worklist ids" do
      XmlUtilities.should_receive(:fetch_url).with("#{XmlUtilities::WL1_URL}/listids").and_return(@list_ids_string)
    ...
    it "should create a new list with wl1_clone set for each v1 list id it retrieves" do
