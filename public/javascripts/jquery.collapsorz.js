...
(function($) {
	$.fn.collapsorz = function(options) {
		...
		var defaults = {
			...
			minimum: 1, // number to show in collapsed form
		...
		return this.each(function() {
			// only execute if there are more than minimum items
                        var num_togglees = $(options.toggle, this).length;
			if(num_togglees > options.minimum) {
				// setup variables
				var $obj = $(this);
				...
				if(options.defaultState == "collapsed") {
					$targets.filter(":lt("+(num_togglees - options.minimum)+")").hide();
				}
				...
				if(options.defaultState == "expanded") {
					...
					$toggler.html("Show all " + num_togglees + ' ' + options.showText);
				}
